...
本仓库基于原项目 [ihmily/StreamCap](https://github.com/ihmily/StreamCap) fork。
以下内容仅记录本 fork 相比原项目新增或调整的修改，后续会持续维护。
...
- `2026-04-29` 新增录制段数功能：启用分段录制后可设置录制段数，达到设定值后自动停止录制并停止监控。
- `2026-04-30` 修复分段录制达到设定段数后未正确停止监控的 Bug。
...
如果你在 Debian 服务器上运行，可以直接使用仓库自带脚本：

```bash
bash start_debian.sh
```

这个脚本会自动检查并安装项目运行需要的本机依赖，创建/复用 `venv`，安装 Python 依赖，并在 `.env` 不存在时自动从 `.env.example` 复制一份。
